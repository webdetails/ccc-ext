...
define([ "./def", "./protovis-compat!" ], function(def, pv) {
    ...
    function data_setDatums(setDatums, keyArgs) {
        ...
        if (def.array.is(setDatums)) for (var i = 0, L = setDatums.length; L > i; ) maybeAddDatum.call(this, setDatums[i++]); else {
    ...
    function cdo_querySpecPredicate(querySpec) {
        function datumQuerySpecPredicate(d) {
            for (var datoms = d.atoms, i = 0; L > i; i++) if (datumFilterPredicate(datoms, querySpec[i])) return !0;
            return !1;
        }
    ...
    function data_where(normalizedQuerySpec, keyArgs) {
        var orderBys = def.array.as(def.get(keyArgs, "orderBy")), datumKeyArgs = def.create(keyArgs || {}, {
            orderBy: null
        }), datumResultsQuery = def.query(normalizedQuerySpec).selectMany(function(normalizedDatumFilter, index) {
    ...
    cdo.Complex.compositeKey = function(complex, dimNames) {
        for (var key = "", D = dimNames.length, keySep = complex.owner.keySep, datoms = complex.atoms, i = 0; D > i; i++) {
    ...
    cdo.Data.add({
        ...
        dimensionsSumAbs: function(dimName, keyArgs) {
            ...
            return null !== value && 0 > value ? -value : value;
        },
    ...
    def.type("cdo.GroupingOper", cdo.DataOper).init(function(linkParent, groupingSpecs, keyArgs) {
        ...
        _getExtensionDatumsMap: function() {
            ...
            if (extensionDataSetsMap) {
                ...
                var baseExtensionDatumsMap = this._linkParent.extensionDatums, isNoDatums = def.query(Object.keys(extensionDataSetsMap)).each(function(dataSetName) {
        ...
        _group: function(datumsQuery) {
            ...
            extensionDatumsMap !== !1 && this._groupSpecRecursive(rootNode, def.query(datumsQuery).array(), extensionDatumsMap, 0);
            return rootNode;
        },
        ...
        _groupSpecRecursiveNormal: function(groupParentNode, groupDatums, groupExtensionDatumsMap, groupSpec, groupSpecIndex) {
            function groupLevelRecursive(levelParentNode, levelDatums, levelExtensionDatumsMap, levelIndex) {
                ...
                for (var childNodes = levelParentNode.children = this._groupLevelDatums(levelSpec, levelParentNode, levelDatums, levelExtensionDatumsMap, !1), i = 0, C = childNodes.length; C > i; i++) {
    ...
    def.type("cdo.LinearInterpolationOper").init(function(baseData, partData, visibleData, catRole, serRole, valDimName, stretchEnds) {
        this._newDatums = [];
    ...
    def.type("cdo.ZeroInterpolationOper").init(function(baseData, partData, visibleData, catRole, serRole, valDimName, stretchEnds) {
        this._newDatums = [];
        this._data = visibleData;
        var qAllCatDatas = catRole.flatten(baseData).children(), serDatas1 = serRole.isBound() ? serRole.flatten(partData, {
            visible: !0,
